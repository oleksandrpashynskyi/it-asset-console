...
@WebServlet(name = "Workstations", urlPatterns = {"/Workstations"})
public class Workstations extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String id = request.getParameter("workID");
        String result = Queryable.queryComputers(id);
        
        response.setContentType("text/html");
        PrintWriter out = response.getWriter();
        out.println("<html><head><link rel='stylesheet' href='styles.css'></head>");
        out.println("<body><div class='container'>");
        out.println("<h2>Workstation Data</h2>");
        out.println(result);
